...
export default function ServiceSection({
  ...
  const projetos = useMemo(
    () =>
      tipoProjeto
        ? PROJETOS.filter((p) => p.tipo === tipoProjeto).slice(0, maxProjetos)
        : [],
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => setProjIdx((i) => (i + 1) % projetos.length), 6000);
  ...
  const onMouseUp: React.MouseEventHandler<HTMLDivElement> = () => { dragging.current = false; };
  const onDoubleClick: React.MouseEventHandler<HTMLDivElement> = () => {
    setScale((s) => (s === 1 ? 2 : 1)); if (scale !== 1) setPos({ x: 0, y: 0 });
  };
  useEffect(() => { resetView(); }, [preview]);
  ...
  const wrapCls =
    isNavy ? "section-navy"
    : isAqua ? "section-aqua"
    : isAccent ? "section-accent"
    : variant === "white" ? "rounded-3xl ring-1 ring-black/5 bg-white"
    : "section-sky";
  ...
  const subCls =
    isDark ? "mt-2 max-w-3xl text-white/85" : "mt-2 max-w-3xl text-brand-900";
  ...
  const kpiValueCls =
    isDark ? "text-2xl font-extrabold text-white" : "text-2xl font-extrabold text-brand-900";
  const kpiLabelCls = isDark ? "text-white/85" : "text-brand-900";
  const kpiHintCls  = isDark ? "mt-1 text-xs text-white/60" : "mt-1 text-xs text-brand-900/70";

  const heading3Cls =
    isDark ? "mb-4 text-xl font-semibold text-white" : "mb-4 text-xl font-semibold text-brand-900";
      ...
      <div className="mx-auto max-w-6xl px-4 py-16 space-y-12">
        ...
        <ul className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {bullets.map((b, i) => (
            <li key={i} className={bulletItemCls}>
              <Check className={isDark ? "mt-1 h-5 w-5 flex-none text-green-300" : "mt-1 h-5 w-5 flex-none text-green-600"} />
              <span className={isDark ? "text-white" : "text-brand-900"}>{b}</span>
            </li>
          ))}
        </ul>
        ...
        {logos && logos.length > 0 && (
          <div className="space-y-3">
            {logosTitle && <h3 className={heading3Cls + " text-center"}>{logosTitle}</h3>}
            <div
                      ...
                      <article className="overflow-hidden rounded-2xl bg-white ring-1 ring-black/5 shadow-sm max-w-3xl mx-auto">
                        <div className="relative w-full h-64 md:h-96 overflow-hidden bg-gray-100">
                          {p.capa ? (
                            <img
                              src={p.capa}
                              alt={p.nome ?? p.titulo ?? "Landing page"}
                        ...
                        <div className="p-4">
                          <div className="text-[11px] uppercase tracking-wide text-gray-500">Landing page</div>
                          <div className="mt-1 text-lg font-semibold text-gray-900">{p.nome ?? p.titulo ?? "Cliente"}</div>
                          {p.nicho && <div className="text-gray-500">{p.nicho}</div>}

                          <div className="mt-3 flex flex-col gap-2 sm:flex-row">
                            <button
                              type="button"
                              onClick={() => p.capa && setPreview(p.capa)}
                              className="rounded-full border px-4 py-2 text-center text-sm font-medium hover:bg-gray-50"
                            >
                              Ver imagens
                            </button>
                            <a
                  ...
                  <div className="mt-3 flex items-center justify-center gap-2">
                    {projetos.map((_, i) => (
                      <button
                        ...
                        className={`h-2.5 w-2.5 rounded-full transition-all ${i === projIdx ? "bg-gray-800" : "bg-gray-300"}`}
                  ...
                  {preview && (
                    ...
                    >
                      <div
                        ...
                        style={{ cursor: dragging.current ? ("grabbing" as const) : ("grab" as const) }}
                      >
                        <button
        ...
        {depoimentoServico && (
          <div className="pb-2 sm:pb-0">
            <Depoimentos servico={depoimentoServico as any} autoMs={3500} />
          </div>
        )}

        {/* CTA */}
        <div className="flex flex-col gap-3 sm:flex-row">
          <a href={cta.primaryHref} target="_blank" rel="noreferrer" className={primaryBtnCls}>
            {cta.primaryLabel}
          </a>
          {cta.secondaryHref && cta.secondaryLabel && (
            <a href={cta.secondaryHref} target="_blank" rel="noreferrer" className={ctaSecondaryCls}>
              {cta.secondaryLabel}
            </a>
          )}
        </div>
...
function Check({ className = "mt-1 h-5 w-5 flex-none text-green-600" }: { className?: string }) {
  return (
